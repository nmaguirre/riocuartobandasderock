Feature: The application responds appropriately to all events that correspond to the various forms of Find a band into the system.
  ...
  Scenario: Search bands on a not empty band's database by name
    ...
    When I search a band with name "Ska-p" and the band exist in the database
    Then the band with name "Ska-p" should be on bands' database

  Scenario: Search bands on a not empty band's database by genre
    Given that the bands' database have 5 entries
    When I search a band with genre "ska/punk" and the band exist in the database
    Then the band with genre "ska/punk" should be on bands' database

  Scenario: Search bands on a not empty band's database by name and genre
    Given that the bands' database have 5 entries
    When I search a band with name "Ska-p" and genre "ska/punk" and the band exist in the database
    Then the band with name "Ska-p" and genre "ska/punk" should be on bands' database
